...
$PHPmodules = array(
	"Основные функции"					=> "modules/functions.php",
...
$JSmodules = array(
	...
	"Основной JS сайта"  				=> "/modules/standart/js/MainModule.js?t=501505122018",
...
$CSSmodules = array(
    ...
    "ProGorodChelny Styles"		=> '/template/standart/style.css?t=271018',
    'Owl Carousel'			=> '/template/index/libs/owl_carousel/owl.carousel.min.css',
    'Youtube Fancybox'		=> '/template/index/libs/fancybox/jquery.fancybox.css',
...
$mobileWidth = isset($_SESSION['full']) && $_SESSION['full'] ? '991px' : 'device-width';
$MOBILE='<meta name="viewport" content="width=' . $mobileWidth . ', initial-scale=1.0, maximum-scale=1" />'.$r;
...
$MENU["mainmenu"]=str_replace("<!--USER-->", $Page["UserAuth"], $MENU["mainmenu"]);
...
foreach ($JSmodules as $name=>$module) {
	if (strpos($module, "http:")===false) {
		if (is_file(trim(explode('?', $module)[0],"/"))) { $GLOBAL["log"].="<i>Подключение JS</i>: скрипт &laquo;".$name."&raquo; подключен<hr>";
...
foreach ($CSSmodules as $name=>$module) {
	if (strpos($module, "http")===false) {
		if (is_file(trim(explode('?', $module)[0],"/"))) { $GLOBAL["log"].="<i>Подключение CSS</i>: стиль &laquo;".$name."&raquo; подключен<hr>";
...
// Вывод шаблона сайта ========================================================================================================================================================
$RENDER='<!DOCTYPE html>'.$r;
$RENDER.='<html lang="ru" dir="ltr">'.$r.'<head>'.$r;
...
$RENDER.='<meta name="yandex-verification" content="c677fe6e55f3a58f" /><meta name="keywords" content=\''.trim($Page["KeyWords"], ",").'\' /><meta name="description" content=\''.trim($Page["Description"], ",").'\' />'.$r;
$RENDER.=$ICON;
$RENDER.=$MOBILE;
...
$RENDER.='<div class="actionBanner ToUp" id="Banner-17-1"><div class="closeban">Закрыть [X]</div></div>';
$RENDER.='<div id="ProPodlogka"></div>';
if( ! empty($_SERVER['HTTP_REFERER']) && preg_match('#//[^/]*prokazan#', $_SERVER['HTTP_REFERER'])) {
    $_SESSION['prokazan_return'] = time();
}
if( ! empty($_SESSION['prokazan_return']) && $_SESSION['prokazan_return'] + 60*15 > time() ) {
    $RENDER .= '<a id="ToPK" href="http://prokazan.ru/" style="display:block;">Вернуться на ProKazan.ru</a>';
}
...
adv.banner(function(webmd) {
 ...
 $RENDER.='<script type="text/javascript">var _acic={dataProvider:10};(function(){var e=document.createElement("script");e.type="text/javascript";e.async=true;var t=document.getElementsByTagName("script")[0];t.parentNode.insertBefore(e,t)})()</script>'. $r.'</body></html>';
